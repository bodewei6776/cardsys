...
module ApplicationHelper
  ...
  def operations_panel(object, operations = [:show, :edit, :destroy, :switch_state])
   array = []
   (array << (link_to raw("<i class = 'icon-search'></i>"), object, :title => "查看")) if operations.include?(:show) 
   ...
   (array << (link_to raw("<i class = 'icon-lock'></i>"), send("switch_state_#{object.class.name.underscore}_path", object), :confirm => "确认要启用么？", :method => :put, :title => "启用")) if operations.include?(:switch_state) && object.disabled? 
   (array << (link_to raw("<i class = 'icon-film'></i>"), send("switch_state_#{object.class.name.underscore}_path", object), :confirm => "确认要禁用么？", :method => :put, :title => "禁用")) if operations.include?(:switch_state) && object.enabled? 
   (array << (link_to raw("<i class = 'icon-edit'></i>"), department_power_index_department_path(object), :title => "权限管理"))  if object.is_a?Department
   raw array.join(" ")
  end
  ...
  def user_menus
    [
     ...
     {:image_offset => "8", :link => "/lockers", :sub_menu => "locker_menu", :display => "储物柜"},
  ...
  def control_helper(field, text, float = :oneline, &block)
    style = case float
    when :oneline
      "clear: both;"
    when :left
