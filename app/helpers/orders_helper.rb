...
module OrdersHelper
  ...
  def display_enable_buttons(order, f = nil)
    ...
    htmls << order_action_button("取消预订", "", "cancel") if order.can_cancel? && current_user.admin?
    htmls << order_action_button("连续取消", "", "cancel_all") if order.can_cancel_all? && current_user.admin?
