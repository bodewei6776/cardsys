...
module OrdersHelper
  ...
  def display_enable_buttons(order, f = nil)
    ...
    if  current_user.menus.include?("取消场地")
    ...
    htmls << order_action_button("连续变更", "", "update_all") if order.can_update_all? && current_user.menus.include?("修改场地")
  ...
  def display_content(book_record)
    ...
    content << "(结: #{order.balances.first.who_balance.try(:user_name) || ""})" if order.balanced?
    content
  end
